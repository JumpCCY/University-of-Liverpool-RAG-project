...
def main(user_query: str) -> str:
    ...
    if category == "requirement":
        ...
        prompting = answer_qualification_constuct(user_query, metadata, qualifications_data)
        answer = LLM_query(prompts.ANSWERER, prompting).message.content # construct the context and query the LLM for an answer
        return answer

    #route to vector database similarity search
    elif category == "general":
        vector_search_results = vector_similarity_search(user_query, n_results=10) # get a search result as a list of dicts with keys "distance", "source_type", and "document"
        prompting = answer_vector_search_construct(user_query, vector_search_results)
